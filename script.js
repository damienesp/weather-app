...
function addWeather(data) {
  ...
  showTemp.innerHTML = `<p class="temp-city">${searchInput.value}, ${data.sys.country}</p>
                        <p class="time-date">${time(data.dt, data.timezone)}</p>
                        <span class="temp-num">${Math.round(data.main.temp)}°C</span>
                        <img class="weather-icon" src="./img/weather_icon/${data.weather[0].icon}.svg"/>`;
  // clean prev temp
  tempDiv.innerHTML = "";
  // clean search result
